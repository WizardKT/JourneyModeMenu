    ...
    private void addResearchSlot() {
        this.addSlotToContainer(new SlotItemHandler(this.researchInventory, 0, 98, 32) {
            ...
            public void putStack(ItemStack stack) {
                // Добавлена проверка && GuiResearchContainer.this.isShiftPressedRightNow
                // Теперь предмет съедается ТОЛЬКО если мы закинули его через Shift!
                if (!stack.isEmpty() && ConfigMain.autoResearch.easy_research_shift && GuiResearchContainer.this.isShiftPressedRightNow) {
                    ...
                    if (cap != null) {
                        if (!player.world.isRemote) {
                            ...
                            if (added > 0 && player instanceof net.minecraft.entity.player.EntityPlayerMP) {
                                // Используем нашу Стену из Nokia 3310 (ResearchKey) для получения чистого прогресса
                ...
                // Если это обычный клик мышкой — предмет спокойно ложится в слот
